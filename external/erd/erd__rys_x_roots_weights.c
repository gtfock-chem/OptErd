...
/* Subroutine */ int erd__rys_x_roots_weights_(integer *nt, integer *ntgqp, 
	integer *ngqp, integer *nmom, doublereal *tval, doublereal *ryszero, 
	doublereal *a, doublereal *b, doublereal *mom, doublereal *dia, 
	doublereal *off, doublereal *row1, doublereal *row2, doublereal *rts, 
	doublereal *wts)
{
    ...
    /* Local variables */
    doublereal c__, d__, f, g;
    integer i__, j, m, n;
    doublereal p, r__, s, t, r1;
    integer ip1, jp1;
    doublereal lim1, lim2, lim3, binc, sinc;
    integer imax, jmax;
    doublereal momi;
    integer iter;
    doublereal tinv, texp, zmom, root;
    integer nrts;
    doublereal zinv, test1, test2, tinv2, scale, sigma, theta, momim1, 
...
/*             ...proceed with higher rows. */
		...
		if (i__ % 2 == 0) {
		    ...
		    for (j = i__; j <= i__3; ++j) {
			jp1 = j + 1;
			row1[j] = (a[jp1] - sigma) * row2[j] + row2[jp1] + b[
				jp1] * row2[j - 1] - theta * row1[j];
		    }
		    ...
		    for (j = i__; j <= i__3; ++j) {
			jp1 = j + 1;
			row2[j] = (a[jp1] - sigma) * row1[j] + row1[jp1] + b[
				jp1] * row1[j - 1] - theta * row2[j];
		    }
...
L3000:
		...
		for (m = j; m <= i__3; ++m) {
		    ...
		    test1 = (d__1 = dia[m], abs(d__1)) + (d__2 = dia[m + 1], 
			    abs(d__2));
...
/*                range. */
		...
		rts[nrts + i__] = root;
		d__1 = a[i__];
		wts[nrts + i__] = wts[nrts + i__] * momzero * (d__1 * d__1);
	    }
	    nrts += *ngqp;
	}
    }
