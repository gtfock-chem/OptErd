  ...
  L11:
    ...
    for (ij = 1; ij <= mij; ++ij)
    {
        ...
        for (kl = 1; kl <= mkl; ++kl)
        {
            qval = q[kl];
            pqplus = pval + qval;
            f0 = pscale * scaleq[kl] / (pval * qval * sqrt (pqplus));
            u1 = (f0 - pval / pqplus * f0 * .333333333333333) / (qval + qval);
            batch[m + 1] = u1;
            batch[m + 2] = 0.;
            batch[m + 3] = 0.;
            batch[m + 4] = 0.;
    ...
    for (ij = 1; ij <= mij; ++ij)
    {
        ...
        for (kl = 1; kl <= mkl; ++kl)
        {
            ...
            else
            {
                tinv = 1. / t;
                t2inv = tinv * .5;
                f1 = t2inv * scale * .5 * sqrt (tinv * 3.141592653589793);
                f2 = t2inv * 3. * f1;
            }
            u0 = pval * pqpinv;
            u1 = -qval * pqpinv;
  ...
  L31:
    ...
    for (ij = 1; ij <= mij; ++ij)
    {
        ...
        for (kl = 1; kl <= mkl; ++kl)
        {
            ...
            pqplus = pval + qval;
            f0 = pscale * scaleq[kl] / (pval * qval * sqrt (pqplus));
            c = u1 * (f0 - qval / pqplus * f0 * .333333333333333);
            batch[m + 1] = c;
            batch[m + 2] = 0.;
            batch[m + 3] = 0.;
            batch[m + 4] = 0.;
...
int erd__sspp_pcgto_block_ (int * nbatch, int * atomic,
                         ...
                         double * scaleq, double * batch)
{
    erd__sspp_pcgto_block (*nbatch, *atomic, *atom12, *atom34,
                           *mij, *mkl,
                           *nij, *nijbeg, *nijend,
                           *nkl, *nklbeg, *nklend,
                           *npgto1, *npgto2,
